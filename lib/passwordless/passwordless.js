				...
				if(valid) {
					var success = function() {
						...
						if(options.successRedirect) {
							...
							var fullUrl = req.protocol + '://' + req.get('host') + pathname + (pathname === "/" ? "": "?id="+req.query.id);
...
//user is the user object
 Passwordless.prototype.requestTokenDirectly = function(req, user, delivery, callback, options) {
 	var self = this;
 	options = options || {};
 	if (!user.id) {
	...
	//not checking for body this time as its not called as middleware
	...
	//we don't need the userfield as its not coming from the client side
	//we don't need a delivery field as its not coming from client side
	//we don't need an origin field

	//its going to set the delivery method to default 
	//if there is no delivery method available or set
	var deliveryMethod = self._defaultDelivery;
	if(delivery && self._deliveryMethods[delivery]) {
		//but if there is, use that delivery method
		deliveryMethod = self._deliveryMethods[delivery];
	}

		if(typeof user === 'string' && user.length === 0) {
			throw new Error('Please provide a valid user (directly)')
		} else if(!deliveryMethod || !user) {
			throw new Error("There is no user or delivery method set")
		}

		//uid is the id of the user we are sending the email to
		...
		//set the lifespan of the token to 1000 hours (41 days as default)
		...
		self._tokenStore.storeOrUpdate(token, uid.toString(), ttl, origin, function(storeError) {
			if(storeError) {
				...
				console.log("SENDING TO: ", user[options.userField])
				deliveryMethod.sendToken(token, uid, user[options.userField], req, function(deliveryError) {
					if(deliveryError) {
						callback(new Error('Error on the deliveryMethod delivery layer: ' + deliveryError));
					} else {
						if(!req.passwordless) {
							req.passwordless = {};
						}
