...
Passwordless.prototype.acceptToken = function(options) {
	...
	return function(req, res, next) {
		...
		// console.log("uid field: ", uidField);
		...
		// console.log("checking for token: ", token, " and uid: ", uid);
		if(token && uid) {
			self._tokenStore.authenticate(token, uid.toString(), function(error, valid, referrer) {
				// console.log("checking valid: ", valid);
				if(valid) {
					// console.log("valid is set")
					var success = function() {
						// console.log("inside success");
						req[self._userProperty] = uid;
						// console.log("req[self._userProperty]: ", req[self._userProperty]);
						if(req.session) {
							// console.log("there is a user session");
							req.session.passwordless = req[self._userProperty];
						}
						if(options.successFlash) {
							// console.log("passwordless-success", options.successFlash);
							req.flash('passwordless-success', options.successFlash);
						}
						// console.log("ORIGIN REDIRECT: ", options.enableOriginRedirect, " referrer: ", referrer);
						if(options.enableOriginRedirect && referrer) {
							// console.log("origin redirect enabled with referrer")
							return self._redirectWithSessionSave(req, res, next, referrer);
						}
						if(options.successRedirect) {
							// console.log("success Redirect is set: ", req.url);
							// next() will not be called, and
							// enableOriginRedirect has priority
							//rebuilds thie redirect uid with the id of the email still as a parameter
							var pathname = url.parse(req.url).pathname
							// console.log("The path name is " + pathname, " the type is: ", typeof pathname)
							if (pathname === "/") {
								// console.log("path is home")
								return self._redirectWithSessionSave(req, res, next, options.successRedirect);
							} else {
								// console.log("path is NOT home - its probably a 'clicked' email so has an id")
								var fullUrl = req.protocol + '://' + req.get('host') + pathname + "?id="+req.query.id;
								// console.log("REDIRECTING TO: ", fullUrl);
								return self._redirectWithSessionSave(req, res, next, fullUrl);	
							}
						}
						next();
					}
...
Passwordless.prototype.restricted = function(options) {
	...
	return function(req, res, next) {
		...
		if(options.failureRedirect) {
			...
			// console.log('failureFlash cannot be used without failureRedirect');
			throw new Error('failureFlash cannot be used without failureRedirect');
		} else if(options.originField) {
			// console.log('originField cannot be used without failureRedirect');
...
/**
 * This is a custom method that allows for passwordless tokens to be generated 
 * but requestTokenDirectly is called from within a function
 * not as middleware on an api.
 * It works almost exactly as requestToken works
 * But the user doesn't provide the information, it is provided by the function calling it
*/

//user is the user object
 Passwordless.prototype.requestTokenDirectly = function(getUserID, toAddress, delivery) {
 	console.log("REQUEST TOKEN DIRECTLY BEING CALLED")
	var self = this;
	var origin = null; //we don't need to tell it where to return to
	return function(req, res, next) {
		// console.log("This function is being called: ", typeof req);
		...
		if(!self._defaultDelivery && Object.keys(self._deliveryMethods).length === 0) {
			throw new Error('passwordless requires at least one delivery method which can be added using passwordless.addDelivery()');
		}

		var user;
		user = toAddress; //this just sets it for the rest of the function
		...
		console.log("GET USER ID BEING CALLED")
		getUserID(user, delivery, function(uidError, uid) {
			if(uidError) {
				...
				console.log("[DIRECT] about to store the token");
				self._tokenStore.storeOrUpdate(token, uid.toString(), ttl, origin, function(storeError) {
					if(storeError) {
						...
						console.log("[DIRECT] about to send the token");
						// console.log("About to call the delivery method: ", typeof req);
						deliveryMethod.sendToken(token, uid, user, req, function(deliveryError) {
							if(deliveryError) {
								...
								console.log("[DIRECT] setting the AUTH");
				...
				// res.end(null, {status:"NEED-VALID-USER"})
				res.end(JSON.stringify({status:"NEED-VALID-USER"}));
			}
		}, req)
	}
}
 ...
 * This is a custom method that allows for passwordless tokens to be generated 
 * but requestTokenDirectly is called from within a function
 * not as middleware on an api. With no ned for req and rese
 * It works almost exactly as requestToken works
 * But the user doesn't provide the information, it is provided by the function calling it
*/

//user is the user object
 Passwordless.prototype.requestTokenDirectlyForUser = function(getUserID, toAddress, delivery) {
 	console.log("REQUEST TOKEN DIRECTLY BEING CALLED")
	var self = this;
	var origin = null; //we don't need to tell it where to return to
	//return function(req, res, next) {
			...
			if(uidError) {
				...
				self._tokenStore.storeOrUpdate(token, uid.toString(), ttl, origin, function(storeError) {
					if(storeError) {
						...
						console.log("[DIRECT] about to send the token");
						// console.log("About to call the delivery method: ", typeof req);
						deliveryMethod.sendToken(token, uid, user, "null", function(deliveryError) {
							if(deliveryError) {
								next(new Error('Error on the deliveryMethod delivery layer: ' + deliveryError));
							} else {
								// if(!req.passwordless) {
					...
					if(storeError) {
						...
						deliveryMethod.sendToken(token, uid, user, req, function(deliveryError) {
							if(deliveryError) {
								...
								req.passwordless.uidToAuth = uid;
								if(options.successFlash) {
									// console.log("WE ARE FLASHING SUCCESS - requestToken");
									req.flash('passwordless-success', options.successFlash);
								}
								next();
							}
...
Passwordless.prototype._redirectWithSessionSave = function(req, res, next, target) {
	...
	if (!req.session || this._skipForceSessionSave) {
		...
		req.session.save(function(err) {
			if (err) {
				// console.log("yup there is an error: ", err)
				return next(err);
			} else {
				// console.log("nope, we are redirecting to: ", target);
				res.redirect(target);
			}
		});
	}
